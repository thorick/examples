 ...
 */
public class HashTable_2SUM_02_Windows_Stanford_Coursera
        extends HashTable_2SUM_Stanford_Coursera_Base {


  private Logger log =
          Logger.getLogger(HashTable_2SUM_02_Windows_Stanford_Coursera.class);
  ...
  public HashTable_2SUM_02_Windows_Stanford_Coursera(FileResult fr) {
    inOrig = fr.array;
    ...
    if (valueLeftCurrent < 0) {
      // the idea first right value to add, will be the one that yields L + R = -10000  AND  L + R <= 10000
      //    so R =  -10000 - L
      valueRightStartIdeal = LOWERSUM - valueLeftCurrent;
      if (isP())
        log.debug("initial valueRightStartIdeal is " + valueRightStartIdeal);

      while (valueRightStartIdeal > valueInputArrayLast) {
        if (isP())
          log.debug("at left index[" + indexLeftWindowStart + "] valueRightStartIdeal=" + valueRightStartIdeal +
                  " exceeds the array max=" + valueInputArrayLast + ", so skip current left and try next");
      ...
      if (isP())
        log.debug("using starting right index indexRightWindowStart[" + indexRightWindowStart + "]=" + inOrig[indexRightWindowStart] +
                ...
                "\n sanity if we had chosen the PREVIOUS right element the first SUM would be: " +
                (inOrig[indexLeftWindowStart] + inOrig[indexRightWindowStart - 1])));
    } else {
      // the ideal first right value to add, will be the first one that yields a value L + R >= 0  AND  L + R <= 10000
      //    as long as inOrig[L} + inOrig{L+1] < 10000 then  R = L+1
    ...
    while (!stop) {
      ...
      if (sum >= LOWERSUM && sum <= UPPERSUM) {
        ...
        HS0.add(sum);
        indexRightCurrent++;
        if (indexRightCurrent <= indexInputArrayLast)
          continue;    // next
  ...
  public static void main(String[] args) {
    ...
    HashTable_2SUM_02_Windows_Stanford_Coursera runner = new HashTable_2SUM_02_Windows_Stanford_Coursera(fr);
  }
