...
public class Kosaraju_SCC {
  ...
  private Logger log =
          Logger.getLogger(Kosaraju_SCC.class);
  ...
  public Kosaraju_SCC(GraphAdjList_Kosaraju_SCC g) {
    this.g = g;
  ...
  public void findSCCs() {
    ...
    ran = true;
    GraphAdjList_Kosaraju_SCC rg = g.reverse();
    ...
    for (int v = LOWEST_VERTEX; v <= highestVertexNumber; v++) {
      if (dfsProcessed[v] == NOT_PROCESSED) {
        ...
        dfs(rg, v, dfsBranchVertices);
      }
    }

    if (isP()) {
      log.debug("DONE with REVERSED GRAPH.  after dfs reverse postOrder=" + printIntArray(postOrder) + "\n\n");
    }
    ...
    for (int v = highestVertexNumber; v > stopAt; v--) {
      ...
      if (dfsProcessed[secondDFSOrder[v]] == NOT_PROCESSED) {
        ...
        dfs(g, secondDFSOrder[v], dfsBranchVertices);

        if (isP())
          log.debug("--   DONE dfs vertex: " + secondDFSOrder[v] + " addSCC component using input '+" +
                dfsBranchVertices.toString() + "'");

        addSCC(componentCount, g.getAdjList(secondDFSOrder[v]), dfsBranchVertices.toString());
  ...
  private void dfs(GraphAdjList_Kosaraju_SCC g, int v, StringBuilder branchVertices) {
   ...
   * @return
   */
  private SCC addSCC(int id, LinkedNode head, String vList) {

    SCC c = new SCC(id, head, vList);
    if (sccList == null) {
      sccList = c;
      return c;
    }
  ...
  public class SCC implements Comparable<SCC> {
    ...
    private SCC(int id, LinkedNode head, String vList) {
      this.id = id;
    ...
    public int compareTo(SCC other) {
      // todo: implement
      return 0;
    }
  }
