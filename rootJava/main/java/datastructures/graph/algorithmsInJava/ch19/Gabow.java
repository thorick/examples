...
public class Gabow {
  ...
  private void scR(int w) {
    ...
    Integer wInteger = new Integer(w);
    s.push(wInteger);  // vertex stack
    p.push(wInteger);
    p("stack="+printStack(s)+"\n");
    ...
    while (t != w);
    p("done with assignment stack is now="+printStack(s)+"\n");
  ...
  private String printStack(Stack<Integer> st) {
    StringBuilder sb = new StringBuilder();
